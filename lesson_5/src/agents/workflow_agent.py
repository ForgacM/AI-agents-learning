...
from ..clients import LLMClient, MultiMCPClient
...
class WorkflowAgent(Agent):
    ...
    - Weather MCP (localhost:8002) - weather API tools
    - DB MCP (localhost:8003) - database cache tools
    """
    ...
    def __init__(
        ...
        Args:
            ...
            mcp_servers: Dict mapping server names to URLs.
                        Default: {"weather": "http://localhost:8002", "db": "http://localhost:8003"}
        """
        super().__init__(name)
        self.llm = LLMClient(llm_model=model)

        # Default MCP servers: weather on 8002, db on 8003
        if mcp_servers is None:
            mcp_servers = {
                "weather": "http://localhost:8002",
                "db": "http://localhost:8003",
            }
        self.mcp_client = MultiMCPClient(servers=mcp_servers)
